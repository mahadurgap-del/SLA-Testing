...
async function runRegression(cfg, params, opts = {}) {
  ...
  try {
    ...
    // curLog/ holds today's full day-log (hour dirs of 5-min buckets; rotated to
    // dmts_logs_N.tar.xz at midnight), so a single end-of-run grab covers every
    // case window — the authoritative pooled evidence; per-case hourLog tarballs
    // are convenience slices. sinceMs pulls rotated tars too if we crossed midnight.
    const sinceMs = Date.parse(state.startedAt || "") || null;
