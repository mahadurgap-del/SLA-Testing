...
| 15 | Cleanup: netem cleared after each case + run-end safety net + pre-run sanitize | 🧪 offline-tested (registry) | Mid-TC3, kill the panel process (`taskkill`), check `tc qdisc show` on the netem VM (rules will remain — expected for a hard kill), then start ANY new run: the `Netem VM clean state` checkpoint must report removing them. |
